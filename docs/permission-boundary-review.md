...
A 2026-08-20 review found that the nine-tool capture did not actually expose `websearch`: OpenCode 1.18.18 hides it for local providers unless a search provider flag is enabled, regardless of the permission entry. Alpine now enables the Exa-backed search route explicitly. The resulting fresh serialized request is 28,629 bytes with ten tool schemas totaling 22,838 bytes; the added capability costs context and sends search queries to an external service. To keep real research usable inside 16K, all model-visible tool results are capped at 500 lines or 12,288 bytes while OpenCode retains the full result for focused local inspection. This is a context bound, not a subject-matter or tool-capability restriction.
