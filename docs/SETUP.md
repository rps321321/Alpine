...
Profile files contain inference material only. Runtime, KV-cache, thread-limit, and request-local n-gram compatibility comes from the shared `config/profile-capabilities.json` contract. Qualification and append-only deployment roles carry lifecycle facts; see `config/promotion-policy.json` and `docs/DEPLOYMENT.md`.
...
The default public-v1 golden-agent harness runs OpenCode directly from Rust in a copied fixture. It machine-checks a multi-file TDD repair, exactly one declared failed read followed by a successful tool effect, and retention of an early exact constraint through context growth. It verifies the effective 16K policy, strips ambient credential variables from the child, rejects malformed or conflicting tool events, runs the versioned tests, rejects protected or unexpected changes, and binds evidence to the exact OpenCode and test executable identities plus the task-suite hash. Promotion Policy requires this exact task and capability set. Checked-in tasks are public fixtures, not hidden evaluation evidence.
