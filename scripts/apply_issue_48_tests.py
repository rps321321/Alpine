...
store = ROOT / "apps/desktop/src-tauri/src/store.rs"
...
if value.count(old) != 1:
    raise RuntimeError("journal export block changed")
store.write_text(value.replace(old, new, 1), encoding="utf-8")

(ROOT / "apps/desktop/src-tauri/tests/desktop_store.rs").write_text(r'''use alpine_desktop::store::{
...
fn multiple_executions_preserve_failure_retry_identity_and_ordered_history() {
    ...
    assert!(detail.messages.iter().any(|message| message.execution_id == first));
    assert!(detail.messages.iter().any(|message| message.execution_id == second));
    assert!(
        detail
            .events
            .iter()
...
fn crash_during_tool_settlement_does_not_invent_a_result() {
    ...
    assert_eq!(
        reopened.get_tool_approval(&approval.id).unwrap().unwrap().state,
        ApprovalState::Interrupted
    );
    assert_eq!(
        reopened
...
fn approval_decision_and_execution_resume_share_one_journal_transaction() {
    ...
    assert_eq!(decision.records[1].sequence, decision.records[0].sequence + 1);
}
...
fn projection_rebuild_is_deterministic_for_transcript_summary_approvals_and_tools() {
    ...
    let before_messages = before
        ...
        .collect::<Vec<_>>();
    assert_eq!(
        store.get_tool_approval(&approval.id).unwrap().unwrap().state,
...
fn deleting_a_task_cascades_journal_and_all_execution_projections() {
    ...
    assert!(store.get_execution(execution_id.as_str()).unwrap().is_none());
...
fn schema_three_history_migrates_with_explicit_unverified_legacy_provenance() {
    ...
    assert_eq!(execution.specification.runtime_identity, "legacy-unverified");
...
fn project_roots_are_canonical_and_unique() {
    ...
    assert_eq!(Path::new(&created.root), project_root.canonicalize().unwrap());
    assert!(
        store
...
(ROOT / "apps/desktop/src-tauri/tests/workspace.rs").write_text(r'''use alpine_control_plane::sanitized_process_environment;
