...
export function App({ desktop }: { desktop: DesktopClient }) {
  ...
  return (
    <div className={`app-shell ${leftRailOpen ? "" : "left-closed"} ${inspectorOpen ? "" : "right-closed"} ${inspectorOpen && inspectorTab === "browser" ? "browser-open" : ""}`}>
      ...
      <main className="workspace" ref={workspaceRef}>
        ...
        {view === "settings" && <SettingsView snapshot={snapshot} downloads={downloads} runtimeControl={runtimeControl} onRuntimeControl={controlRuntime} onSave={saveSettings} onUseActive={useActiveRuntimeModel} onSelectInstalled={selectInstalledModel} onClearBrowserData={() => desktop.browser.clearData()} />}
      ...
      {inspectorOpen && <ContextInspector browser={desktop.browser} tab={inspectorTab} setTab={setInspectorTab} snapshot={snapshot} bootstrapError={bootstrapError} selected={view === "models" ? selected : null} selectedArtifact={view === "models" ? selectedArtifact : undefined} selectedInstalled={view === "models" && selectedInstalled} assessment={view === "models" ? assessment : null} placement={view === "models" ? placement : null} metrics={rendererMetrics} defaultSaved={defaultSaved} saveDefault={saveDefault} chooseArtifact={chooseArtifact} downloadState={downloadState} downloadProgress={downloadProgress} downloadSelected={downloadSelected} cancelDownload={cancelSelectedDownload} taskDetail={taskDetail} files={workspaceFiles} workspaceRead={workspaceRead} clearWorkspaceRead={() => setWorkspaceRead(null)} openFile={openWorkspaceFile} hardwareLine={hardwareLine} />}
    </div>
  );
}
...
function SettingsView({ snapshot, downloads, runtimeControl, onRuntimeControl, onSave, onUseActive, onSelectInstalled, onClearBrowserData }: { snapshot: BootstrapSnapshot | null; downloads: DownloadedModel[]; runtimeControl: { state: "idle" | "running" | "error"; message?: string }; onRuntimeControl: (action: "start" | "stop") => Promise<void>; onSave: (update: SettingsUpdate) => Promise<void>; onUseActive: () => Promise<void>; onSelectInstalled: (filename: string) => Promise<void>; onClearBrowserData: () => Promise<void> }) {
  ...
  const [browserDataState, setBrowserDataState] = useState<{ clearing: boolean; message?: string; error?: boolean }>({ clearing: false });
  ...
  const save = async (event: FormEvent) => {
    ...
    try { await onSave({ installRoot, evaluationRepositoryRoot: evaluationRoot, defaultProfile: profile, localMetricsEnabled: localMetrics, browserAllowedHosts: snapshot?.settings.browserAllowedHosts ?? [] }); setSaveState({ state: "done", message: "Settings saved and runtime rechecked." }); }
...
function ContextInspector({ browser, tab, setTab, snapshot, bootstrapError, selected, selectedArtifact, selectedInstalled, assessment, placement, metrics, defaultSaved, saveDefault, chooseArtifact, downloadState, downloadProgress, downloadSelected, cancelDownload, taskDetail, files, workspaceRead, clearWorkspaceRead, openFile, hardwareLine }: {
