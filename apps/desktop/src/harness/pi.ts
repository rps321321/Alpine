...
import type { DesktopClient, MessageRole, TaskMessage, ToolApproval } from "../desktop";
...
const searchFilesSchema = Type.Object({
  query: Type.String({ description: "Exact text to find inside project files" }),
  limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 500 })),
});
const editFileSchema = Type.Object({
  path: Type.String({ description: "Project-relative path to an existing UTF-8 file" }),
  oldText: Type.String({ description: "Exact unique text to replace" }),
  newText: Type.String({ description: "Replacement text" }),
});
const runCommandSchema = Type.Object({
  command: Type.String({ description: "PowerShell command to run at the Selected Project root" }),
...
export class PiHarness {
  ...
  constructor(config: PiLocalModelConfig, dependencies: PiHarnessDependencies = {}) {
    const tools = dependencies.taskId && dependencies.desktop
      ? createProjectTools(
          dependencies.taskId,
...
function createProjectTools(
  ...
  const searchFilesTool: AgentTool<typeof searchFilesSchema> = {
      ...
      description: "Search for exact text inside UTF-8 files in the Selected Project.",
      parameters: searchFilesSchema,
      async execute(_toolCallId, { query, limit }) {
        const matches = await desktop.searchProjectFiles(taskId, query, limit ?? 200);
  ...
  const editFileTool: AgentTool<typeof editFileSchema> = {
      ...
      async execute(toolCallId, edit, signal) {
        const proposal = { path: edit.path, oldText: edit.oldText, newText: edit.newText };
        ...
        const result = await desktop.editProjectFile(taskId, approval.id, proposal);
        return {
  ...
  const runCommandTool: AgentTool<typeof runCommandSchema> = {
      ...
      async execute(toolCallId, input, signal, onUpdate) {
        const shell = { command: input.command, timeoutSeconds: input.timeoutSeconds ?? 120 };
        const approval = await desktop.requestToolApproval({
          taskId,
          toolCallId,
          operation: "shell",
  ...
  return [listFilesTool, readFileTool, searchFilesTool, editFileTool, runCommandTool];
}
...
async function waitForApproval(
  ...
  for (;;) {
    ...
    if (approval.state === "denied") throw new Error("Operator denied the proposed operation");
...
function restoreMessages(
  ...
  return history.flatMap((message): AgentMessage[] => {
    ...
    if (message.role === "user") {
      return [{ role: "user", content: message.content, timestamp: message.createdAtMs }];
    }
    return [
      {
        role: "assistant",
